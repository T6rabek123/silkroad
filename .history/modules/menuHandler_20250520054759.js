  ...
  bot.on('callback_query', callbackQuery => {
    ...
    // --- User: Review Food Item (Start) ---
      ...
      if (allItems.size === 0) {
        bot.sendMessage(
          chatId,
          '😕 There are currently no dishes available for rating.'
        );
        bot.answerCallbackQuery(callbackQuery.id);
        return;
      }
      ...
      inline_keyboard.push([
        { text: '◀️ Back', callback_data: 'main_menu_from_menu' },
      ]);

      bot.sendMessage(chatId, '⭐ Which dish would you like to rate?', {
      ...
      bot.sendMessage(
        chatId,
        `How many stars would you give to "${itemName}"? (Enter a number from 1 to 5):`
      );
      bot.answerCallbackQuery(callbackQuery.id);
    ...
    // --- Admin: Manage Food Items (Master List) ---
      ...
      if (menuData.food_items.length < initialLength) {
        ...
        bot.answerCallbackQuery(callbackQuery.id, { text: "Dish deleted!" });
        // Refresh the list:
  ...
  bot.on('message', msg => {
    ...
    // --- User Food Review ---
      ...
      if (isNaN(rating) || rating < 1 || rating > 5) {
        bot.sendMessage(
          chatId,
          '⚠️ The rating must be a number from 1 to 5. Please re-enter or cancel with /cancel.'
        );
        return;
      }
      ...
      bot.sendMessage(
        chatId,
        `⭐ ${rating} stars accepted. Now you can write a short comment for "${userState.data.itemName}" (optional, type /skip to skip):`
      );
    ...
    if (
      ...
      if (text.toLowerCase() === '/save_menu') {
        ...
        if (items.length === 0) {
          bot.sendMessage(
            chatId,
            `⚠️ No dishes entered for ${day}. Please enter at least one dish to save the menu or cancel with /cancel_admin_op.`
          );
          return;
        }
        const menuData = readData('menu.json');
        ...
        if (parts.length < 2 || parts.length > 3) {
          bot.sendMessage(
            chatId,
            "⚠️ Incorrect format. Please enter in the format `Dish name,Price,Description (optional)`. Or save with /save_menu."
          );
          return;
        }
        const newItem = {
        ...
        if (!newItem.name || !newItem.price) {
          bot.sendMessage(
            chatId,
            '⚠️ Dish name and price are required. Please re-enter.'
          );
          return;
        }
        ...
        bot.sendMessage(
          chatId,
          `➕ "${newItem.name}" added. Add more or save with /save_menu.`
        );
      }
    }
