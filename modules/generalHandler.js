module.exports = ({
  ...
  bot.onText(/\/cancel/, msg => {
    ...
    bot.sendMessage(
      chatId,
      '❌ Operation cancelled. You can use the menu below to start again.'
    );
  });
  ...
  bot.on('message', msg => {
    ...
    switch (text) {
      ...
      case '💬 Contact Admin':
        ...
        bot.sendMessage(
          chatId,
          '✍️ Please, only tech part of the bot. Write your message @T6rabek'
        );
        break;
      case '🛠 Admin Panel':
        if (isAdmin(userId)) {
          const adminPanelMessage =
            '🛠 Welcome to the Admin Panel! To connect full admin panel send /admin';
          const adminKeyboard = {
            inline_keyboard: [
              [
                {
